...
const phoneNumber = "971555525061"
    ...
    <div className="fixed bottom-6 right-6 z-50 flex flex-col items-center gap-3">
      {bubbles.map((b) => (
        <motion.a
          ...
          aria-label={b.label}
          className={`relative flex items-center justify-center w-14 h-14 rounded-full ${b.bg} text-white shadow-lg hover:shadow-xl transition-shadow`}
          initial={{ scale: 0, opacity: 0 }}
          animate={{ scale: 1, opacity: 1 }}
          transition={{ delay: b.delay, type: "spring", stiffness: 260, damping: 20 }}
          whileHover={{ scale: 1.12 }}
          whileTap={{ scale: 0.95 }}
        >
          {b.icon}
          <span className={`absolute inset-0 rounded-full ${b.pulse} animate-ping opacity-20`} />
        </motion.a>
      ))}
    </div>
  )
